...
use crate::plugin::ext::state_context::StateContext;
...
/// The test for `PluginTestCase::StateContextBasic`.
pub fn test_state_context_basic(library: &PluginLibrary, plugin_id: &str) -> Result<TestStatus> {
    let host = Host::new();
    let plugin = library
        .create_plugin(plugin_id, host.clone())
        .context("Could not create the plugin instance")?;
    ...
    plugin.init().context("Error during initialization")?;
    let state_context = match plugin.get_extension::<StateContext>() {
        Some(state_context) => state_context,
        None => {
            return Ok(TestStatus::Skipped {
                details: Some(format!(
                    "The plugin does not implement the '{}' extension.",
                    StateContext::EXTENSION_ID.to_str().unwrap(),
                )),
            });
        }
    };
    ...
    // Empty load must fail for every context type.
    for context_type in [
        CLAP_STATE_CONTEXT_FOR_PRESET,
        CLAP_STATE_CONTEXT_FOR_DUPLICATE,
        CLAP_STATE_CONTEXT_FOR_PROJECT,
    ] {
        if state_context.load(&[], context_type).is_ok() {
            anyhow::bail!(
                "The plugin returned true when 'clap_plugin_state_context::load()' was called \
                 with an empty state for context type {context_type}."
            );
        }
    }

    // Save+load roundtrip per context type.
    ...
    for context_type in [
        ...
        let blob = state_context
            .save(context_type)
            .with_context(|| format!("save() failed for context type {context_type}"))?;
        state_context
            .load(&blob, context_type)
            .with_context(|| format!("load() failed for context type {context_type}"))?;
        sizes.push((context_type, blob.len()));
    }
    ...
    Ok(TestStatus::Success {
        details: Some(format!(
            "Roundtripped state-context sizes: preset={}, duplicate={}, project={}",
